...
```bash
# If you get "named port 'http'" error, use these alternatives:

# Option 1: Get the correct service name first
kubectl get svc -n default
kubectl port-forward svc/<actual-service-name> -n default 8080:8080

# Option 2: Port forward directly to pod
kubectl get pods -n default
kubectl port-forward pod/<pod-name> -n default 8080:8080

# Option 3: Use port number instead of name
kubectl port-forward svc/<service-name> -n default 8080:8080
```

**ArgoCD sync issues:**
```bash
# Force sync ArgoCD application
argocd app sync <app-name> --server localhost:8080

# Check application status
argocd app get <app-name> --server localhost:8080
```
